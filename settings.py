...
homepage_list = (CONST_HOMEPAGE_DEFAULT
    ...
    , u'嘉義慈濟 https://app.tzuchi.com.tw/tchw/opdreg/SecList_CY.aspx'
    )
#    , u'蘇州慈濟 http://www.tzuchi.com.cn/menzhenzhongxin/yiliaofuwu/menzhenguahao/'
    ...
    if save_ret:
        ...
        if hasattr(sys, 'frozen'):
            ...
            if platform.system() == 'Darwin':
                ...
                subprocess.Popen("./joint", shell=True, cwd=working_dir)
            if platform.system() == 'Linux':
                print("execute linux binary")
                subprocess.Popen("./joint", shell=True, cwd=working_dir)
            if platform.system() == 'Windows':
                print("execute .exe binary.")
                subprocess.Popen("joint.exe", shell=True, cwd=working_dir)
        else:
            ...
            try:
                ...
                s=subprocess.Popen([interpreter_binary, 'joint.py'], cwd=working_dir)
            except Exception as exc:
                print('try', interpreter_binary_alt)
                try:
                    s=subprocess.Popen([interpreter_binary_alt, 'joint.py'], cwd=working_dir)
                except Exception as exc:
                    msg=str(exc)
                    print("exeption:", msg)
...
def PreferenctTab(root, config_dict, language_code, UI_PADDING_X):
    ...
    # Dr Name
    global lbl_dr_name
    lbl_dr_name = Label(frame_group_header, text=translate[language_code]['dr_name'])
    ...
    # User ID
    global lbl_user_id
    lbl_user_id = Label(frame_group_header, text=translate[language_code]['user_id'])
    ...
    # Visit Time
    global lbl_visit_time
    lbl_visit_time = Label(frame_group_header, text=translate[language_code]['visit_time'])
